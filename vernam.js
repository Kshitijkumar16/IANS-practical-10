...
function Vernam_Encrypt() {
    ...
    if(key.length < plaintext.length){ alert("The key must be atleast the length of ciphertext. Try again!"); return; }
    ciphertext = "";
...
function Vernam_Decrypt(f){
    ...
    if(key.length < ciphertext.length){ alert("The key must be atleast the length of ciphertext. Try again!"); return; }
    plaintext = "";
